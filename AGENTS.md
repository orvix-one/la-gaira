...
src/infrastructure/data/    # Port SalesSource y sus adapters (parquet, ERP futuro). Ningún otro layer importa un adapter directo.
src/ui/                     # Componentes presentacionales y wrappers de charts. Sin fetch de datos.
data/raw/                   # Fuente sin procesar (Excel de muestra). Se versiona.
data/processed/             # Artefactos derivados (parquet). Gitignored, se regenera con la ETL.
etl/                        # Scripts de ETL (Excel → Parquet). Vacío hasta la fase de datos.
...
2. El contrato de datos (`src/domain/sales`) es la frontera sagrada. La UI **nunca** lee columnas crudas del Excel/ERP; solo consume el contrato canónico.
3. La obtención de datos pasa **siempre** por el port `SalesSource` (`src/infrastructure/data`). Hoy el adapter será Parquet; mañana el ERP. Ningún componente importa un adapter directamente.
...
- No introduce lecturas directas de Excel/ERP fuera de `src/infrastructure/data`.
